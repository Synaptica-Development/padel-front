  ...
  const fetchBookingHistory = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/Courts/book-history?Page=${page}&PageSize=${pageSize}`,
        {
          method: 'GET',
          headers: {
            'accept': '*/*',
  ...
  return (
    <div className="history-container">
      <div className="history-header">
        <h2 className="history-title">Booking History</h2>
        <p className="history-subtitle">{bookings.length} booking{bookings.length !== 1 ? 's' : ''} found</p>
      </div>
